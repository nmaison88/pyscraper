...
def checkCsv(csvName):
  try:
    csvNamelocation = basedir+'/PlaceCsvHere/'+csvName
    ...
    onlyfiles = [f for f in listdir(basedir+'/PlaceCsvHere') if isfile(join(basedir+'/PlaceCsvHere', f))]
...
def scrape(original_url, csvName = "email.csv"):
  ...
  resultLocation = basedir+'/Results/'+csvName
